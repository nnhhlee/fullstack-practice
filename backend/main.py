...
def create_post(data: PostCreate, db: Session = Depends(get_db)):
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail="게시글 업로드 실패")
...
def update_post(post_id: int, data: PostUpdate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail="게시글 수정 실패")
...
def delete_post(post_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail="게시글 삭제 실패")
